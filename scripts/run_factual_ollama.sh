...
for t in "${TESTS[@]}"; do
    echo "=============================================="
    echo " Running tests.factual.ollama.$t"
    echo "=============================================="
    if ! python -m "tests.factual.ollama.$t" "$IMAGE_PATH"; then
        echo "FAILED: tests.factual.ollama.$t"
        failed=$((failed + 1))
    fi
    echo
done
...
echo "All Ollama factual tests passed."
